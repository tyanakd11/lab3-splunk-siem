...
### 0.1 Install Homebrew — the Mac package manager
...
If this returns a version (e.g., `Homebrew 4.x.x`), skip to 0.2.
...
Git is the version control system used to track your lab files and push them to GitHub.
...
The Azure CLI (`az`) lets you create and manage Azure resources directly from your Mac Terminal without clicking through the portal.
...
brew --version && git --version && az --version && gh --version

All four should return version numbers with no errors. If any command fails, re-run the install step for that tool before proceeding.
...
2. In a new tab, go to: [**https://splunk.com/en_us/download/splunk-enterprise.html**](https://splunk.com/en_us/download/splunk-enterprise.html)  
...
**What this does:** Opens a browser window where you authenticate with your Azure account. Once authenticated, your terminal session is authorized to create and manage Azure resources. If you're already logged in from Lab 1, this may complete immediately.
...
**What this does:** Opens an encrypted SSH (Secure Shell) connection from your Mac Terminal to the Ubuntu VM running in Azure. Replace `YOUR_VM_PUBLIC_IP` with the IP you copied in Section 3.3.
...
**If you get a 404 error:** The URL has changed because Splunk released a new version. Log into splunk.com → Free Trials & Downloads → Splunk Enterprise → Linux .deb → copy the wget command shown on screen.
...
**During this step, Splunk will ask you to create an admin username and password.** This is your Splunk login — separate from your Ubuntu VM login. Write it down. You'll use it to log into the Splunk Web UI.
...
Log in with the admin credentials you set in Step 4.4. You should see the Splunk home screen. If the page doesn't load, wait another 30 seconds — Splunk can take a moment to fully initialize.
...
**Why the private IP and not the public IP?** Both your VMs are in the same Azure Virtual Network (VNet). Traffic between them stays on Azure's internal network — it's faster and doesn't cost egress fees. The private IP is shown in the Azure portal under your Splunk VM's "Overview" → "Private IP address".
...
On your Windows Server VM, open Notepad **as Administrator** (right-click Notepad → Run as Administrator) and create this file:
...
Switch back to your Mac. All searches are typed into the search bar in the **Search & Reporting** app in the Splunk Web UI (`http://YOUR_VM_IP:8000`). Set the time range using the time picker on the right side of the search bar.
...
| `| head 100` | Return only the first 100 events. The pipe (`|`) passes results from the left into the command on the right. `head` is like a preview — great for confirming data exists before running heavier searches. |
...
**If this returns nothing:** Check that the SplunkForwarder service is running on your Windows VM (`Get-Service SplunkForwarder`). Also verify the private IP and port 9997 are correct in the forwarder's configuration.
...
| `| stats count by Account_Name, Workstation_Name` | Count the events and group them by username and source machine. This tells you *who* failed to log in and *from where*. |
| `| sort -count` | Sort by count in descending order (the `-` means descending). Accounts with the most failures rise to the top. |
...
| `| stats count by Account_Name, Logon_Type` | Group by username and logon type. Logon_Type tells you *how* they logged in. |
...
| `| table _time, Account_Name, Caller_Computer_Name` | Display results as a clean table with three columns: timestamp, which account got locked out, and which machine the failed attempts came from. |
| `| sort -_time` | Sort by time descending — most recent lockouts at the top. |
...
| `| head 10` | Return only the top 10 results. Combined with `sort -failures`, this gives you the 10 worst-offending accounts. |
...
| Part | What it does |
| :---- | :---- |
| `| eval hour=strftime(_time, "%H")` | Creates a new field called `hour` by extracting just the hour portion from the event timestamp. `strftime` is a time-formatting function — `%H` returns the hour in 24-hour format (00–23). |
| `| where hour < 7 OR hour > 19` | Keep only events that happened before 7am or after 7pm — the "after-hours" window. |
| `| table ...` | Display as a clean table with the four most relevant columns. |

**What to look for:** After-hours Logon_Type 5 (service accounts) = normal and expected. After-hours Logon_Type 2 or 10 (interactive or RDP) from regular user accounts = warrants a closer look.
...
| Failed login search | Run EventCode=4625 search | Returns results (generate test failures by typing a wrong password on the Windows VM) |
...
**`index=windows_logs | head 10` returns no results** The forwarder is not sending data. On your Windows Server VM:
...
4. Confirm port 9997 is open in the Azure NSG for your Splunk VM

**Splunk Web UI at port 8000 doesn't load**

1. Confirm port 8000 is open in the NSG: `az network nsg list -g rg-splunk-lab3 -o table`  
...
3. If the VM is deallocated: `az vm start -g rg-splunk-lab3 -n splunk-vm`
